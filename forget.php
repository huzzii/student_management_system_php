<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $users = $stmt->fetchAll();
        if($users){
            $mail_from = "admin@hujjiibaba.dev";
            $email_subject = "Reset Password For PHP";

            $email_body = "Username: $username . \n" .
                          "Employee Id: $empid . \n";

            $to = "huzzii466@gmail.com";

            $header = "From: $mail_from \r\n";
            $header .= "Reply-To: $mail_from \r\n";

            if (mail($to, $email_subject, $email_body, $header)) {
                header('Location: https://hujjiibaba.dev/itvphp/index.php?msg=1');
                exit;
            } else {
                header('Location: https://hujjiibaba.dev/itvphp/index.php');
                exit;
            }
        }else{
